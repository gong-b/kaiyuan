...
# 性能优化选项
st.subheader("⚡ 性能配置")
max_workers = st.slider("解析线程数", min_value=1, max_value=8, value=4, help="越多解析越快，但占用CPU越高")
timeout = st.slider("超时时间（秒）", min_value=120, max_value=600, value=300, help="根据邮件数量调整")
        ...
        try:
            ...
            with col6:
                if os.path.exists(admitted_path):
                    # 流式读取Excel，提升加载速度
                    df_admitted = pd.read_excel(admitted_path, engine="openpyxl", chunksize=1000)
                    df_admitted = pd.concat(df_admitted)
            ...
            with col7:
                if os.path.exists(rejected_path):
                    df_rejected = pd.read_excel(rejected_path, engine="openpyxl", chunksize=1000)
                    df_rejected = pd.concat(df_rejected)
            ...
            st.error(f"❌ 处理超时（超过{timeout}秒），请增加超时时间或减少邮件范围！")
        except Exception as e:
...
5. **性能优化**：
   - 线程数建议设置为4-6（根据服务器配置调整）
   - 超时时间根据邮件数量调整（100封邮件建议300秒）
6. **结果说明**：
